// Do not edit this file. It was auto-generated by saveApiMocks.ts

import { Pool } from '../../pool.types'

export const b_50sDAI_50wstETHMock = {
  id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063',
  address: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9',
  name: 'Balancer 50sDAI-50wstETH',
  version: 4,
  owner: '0xba1ba1ba1ba1ba1ba1ba1ba1ba1ba1ba1ba1ba1b',
  swapFeeManager: '0xba1ba1ba1ba1ba1ba1ba1ba1ba1ba1ba1ba1ba1b',
  pauseManager: null,
  poolCreator: null,
  decimals: 18,
  factory: '0x6cad2ea22bfa7f4c14aae92e47f510cd5c509bc7',
  symbol: 'B-50sDAI-50wstETH',
  createTime: 1696322590,
  type: 'WEIGHTED',
  chain: 'GNOSIS',
  protocolVersion: 2,
  tags: ['INCENTIVIZED'],
  hasErc4626: true,
  hasNestedErc4626: false,
  hasAnyAllowedBuffer: false,
  liquidityManagement: {
    disableUnbalancedLiquidity: null,
  },
  hook: null,
  dynamicData: {
    poolId: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063',
    swapEnabled: true,
    totalLiquidity: '4210459.79',
    totalShares: '62138.573876727349026997',
    fees24h: '787.83',
    surplus24h: '0.00',
    swapFee: '0.003',
    volume24h: '262610.31',
    holdersCount: '57',
    isInRecoveryMode: false,
    isPaused: false,
    aprItems: [
      {
        id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-swap-apr-7d',
        title: 'Swap fees APR (7d)',
        apr: 2.987370595808505,
        type: 'SWAP_FEE_7D',
        rewardTokenSymbol: null,
        rewardTokenAddress: null,
      },
      {
        id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-swap-apr-24h',
        title: 'Swap fees APR (24h)',
        apr: 0.0341480868443069,
        type: 'SWAP_FEE_24H',
        rewardTokenSymbol: null,
        rewardTokenAddress: null,
      },
      {
        id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-swap-apr',
        title: 'Swap fees APR',
        apr: 0.0341480868443069,
        type: 'SWAP_FEE',
        rewardTokenSymbol: null,
        rewardTokenAddress: null,
      },
      {
        id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-sDAI-yield-apr',
        title: 'sDAI APR',
        apr: 0.06775185510879438,
        type: 'IB_YIELD',
        rewardTokenSymbol: 'sDAI',
        rewardTokenAddress: '0xaf204776c7245bf4147c2612bf6e5972ee483701',
      },
      {
        id: '0x4d13d387f372dbe5125b7d78e75094ac85b31edc-0x7ef541e2a22058048904fe5744f9c7e4c57af717-balgauge-BAL-apr',
        title: 'BAL reward APR',
        apr: 0.04745139109663624,
        type: 'VEBAL_EMISSIONS',
        rewardTokenSymbol: 'BAL',
        rewardTokenAddress: '0x7ef541e2a22058048904fe5744f9c7e4c57af717',
      },
      {
        id: '0x4d13d387f372dbe5125b7d78e75094ac85b31edc-0x7ef541e2a22058048904fe5744f9c7e4c57af717-balgauge-BAL-apr-boost',
        title: 'BAL reward APR',
        apr: 0.07117708664495437,
        type: 'STAKING_BOOST',
        rewardTokenSymbol: 'BAL',
        rewardTokenAddress: '0x7ef541e2a22058048904fe5744f9c7e4c57af717',
      },
      {
        id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-wstETH-yield-apr',
        title: 'wstETH APR',
        apr: 0.01460824398353367,
        type: 'IB_YIELD',
        rewardTokenSymbol: 'wstETH',
        rewardTokenAddress: '0x6c76971f98945ae98dd7d4dfca8711ebea946ea6',
      },
      {
        id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-swap-apr-30d',
        title: 'Swap fees APR (30d)',
        apr: 0.6688650300004879,
        type: 'SWAP_FEE_30D',
        rewardTokenSymbol: null,
        rewardTokenAddress: null,
      },
    ],
  },
  allTokens: [
    {
      id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-0x6c76971f98945ae98dd7d4dfca8711ebea946ea6',
      address: '0x6c76971f98945ae98dd7d4dfca8711ebea946ea6',
      name: 'Wrapped liquid staked Ether 2.0 from Mainnet',
      symbol: 'wstETH',
      decimals: 18,
      isNested: false,
      isPhantomBpt: false,
      isMainToken: true,
    },
    {
      id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-0xaf204776c7245bf4147c2612bf6e5972ee483701',
      address: '0xaf204776c7245bf4147c2612bf6e5972ee483701',
      name: 'Savings xDAI',
      symbol: 'sDAI',
      decimals: 18,
      isNested: false,
      isPhantomBpt: false,
      isMainToken: true,
    },
  ],
  staking: {
    id: '0x4d13d387f372dbe5125b7d78e75094ac85b31edc',
    type: 'GAUGE',
    chain: 'GNOSIS',
    address: '0x4d13d387f372dbe5125b7d78e75094ac85b31edc',
    gauge: {
      id: '0x4d13d387f372dbe5125b7d78e75094ac85b31edc',
      gaugeAddress: '0x4d13d387f372dbe5125b7d78e75094ac85b31edc',
      version: 2,
      status: 'PREFERRED',
      workingSupply: '7586.766231120025294488',
      otherGauges: [],
      rewards: [
        {
          id: '0x4d13d387f372dbe5125b7d78e75094ac85b31edc-0x7ef541e2a22058048904fe5744f9c7e4c57af717-balgauge',
          rewardPerSecond: '0.000799083516971379',
          tokenAddress: '0x7ef541e2a22058048904fe5744f9c7e4c57af717',
        },
      ],
    },
    aura: {
      id: '0x49aadc30b5ccc57bddd55ac8bd7d8db7cf1f2b8b',
      apr: 0.2185,
      auraPoolAddress: '0x49aadc30b5ccc57bddd55ac8bd7d8db7cf1f2b8b',
      auraPoolId: '19',
      isShutdown: false,
    },
  },
  userBalance: {
    totalBalance: '0',
    totalBalanceUsd: 0,
    walletBalance: '0',
    walletBalanceUsd: 0,
    stakedBalances: [],
  },
  nestingType: 'NO_NESTING',
  poolTokens: [
    {
      id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-0x6c76971f98945ae98dd7d4dfca8711ebea946ea6',
      chain: 'GNOSIS',
      chainId: 100,
      address: '0x6c76971f98945ae98dd7d4dfca8711ebea946ea6',
      decimals: 18,
      name: 'Wrapped liquid staked Ether 2.0 from Mainnet',
      symbol: 'wstETH',
      priority: 0,
      tradable: true,
      isErc4626: false,
      index: 0,
      balance: '550.1197157687078',
      balanceUSD: '2102827.112328728',
      priceRate: '1.0',
      weight: '0.5',
      hasNestedPool: false,
      isAllowed: true,
      priceRateProvider: '0x0000000000000000000000000000000000000000',
      logoURI: 'https://assets.coingecko.com/coins/images/18834/large/wstETH.png?1633565443',
      priceRateProviderData: null,
      nestedPool: null,
      isBufferAllowed: true,
      underlyingToken: null,
      erc4626ReviewData: null,
    },
    {
      id: '0xbc2acf5e821c5c9f8667a36bb1131dad26ed64f9000200000000000000000063-0xaf204776c7245bf4147c2612bf6e5972ee483701',
      chain: 'GNOSIS',
      chainId: 100,
      address: '0xaf204776c7245bf4147c2612bf6e5972ee483701',
      decimals: 18,
      name: 'Savings xDAI',
      symbol: 'sDAI',
      priority: 0,
      tradable: true,
      isErc4626: true,
      index: 1,
      balance: '1832730.1622389015',
      balanceUSD: '2107639.686574737',
      priceRate: '1.0',
      weight: '0.5',
      hasNestedPool: false,
      isAllowed: true,
      priceRateProvider: '0x0000000000000000000000000000000000000000',
      logoURI:
        'https://assets.coingecko.com/coins/images/32066/large/sDAI_Logo_%281%29.png?1696292026',
      priceRateProviderData: null,
      nestedPool: null,
      isBufferAllowed: false,
      underlyingToken: {
        chain: 'GNOSIS',
        chainId: 100,
        address: '0xe91d153e0b41518a2ce8dd3d7944fa863463a97d',
        decimals: 18,
        name: 'Wrapped XDAI',
        symbol: 'WXDAI',
        priority: 0,
        tradable: true,
        isErc4626: false,
        isBufferAllowed: true,
        logoURI:
          'https://assets.coingecko.com/coins/images/14584/large/wrapped-xdai-logo.png?1696514264',
      },
      erc4626ReviewData: {
        reviewFile: './sDaiReview.md',
        summary: 'safe',
        warnings: ['buffer'],
      },
    },
  ],
} as unknown as Pool
