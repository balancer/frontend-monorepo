...
export function useUserAccountLogic() {
  ...
  useAsyncEffect(async () => {
    ...
    // We avoid checking some wallets that are not EOP but not all of them
    // (e.g. hw wallets can be connected through browser wallets)
    if (['safe', 'walletConnect', 'ledger', 'trezor'].includes(result.connector?.id || '')) return
