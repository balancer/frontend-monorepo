...
test.describe('Reliquary page at /mabeets', () => {
  ...
  test('Can create a new relic and then navigate to add liquidity for the created relic', async ({
    page,
  }) => {
    await test.step('Create new Relic', async () => {
      ...
      await expect(
        page.getByText('A new Relic will be created with this add liquidity'),
      ).toBeVisible()
    ...
    await test.step('Add liquidity to created Relic', async () => {
