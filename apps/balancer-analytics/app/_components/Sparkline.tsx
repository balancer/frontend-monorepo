...
export function Sparkline({ values, width = 120, height = 36, stroke = 'green.400', smooth = true }: Props) {
  ...
  const { d, area } = useMemo(() => {
    ...
    const pts: [number, number][] = values.map((v, i) => [i * stepX, height - ((v - min) / range) * (height - 4) - 2])
  ...
  return (
    <Box as="svg" display="block" h={`${height}px`} overflow="visible" viewBox={`0 0 ${width} ${height}`} w={`${width}px`}>
      <defs>
