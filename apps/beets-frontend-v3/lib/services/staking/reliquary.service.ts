...
import {
  ReliquaryStakingPendingRewardAmount,
  StakingPendingRewardAmount,
} from '~/lib/services/staking/staking-types'
...
export class ReliquaryService {
  ...
  public async getPendingRewards({
    ...
    return {
      ...
      fBEETSTotalBalance: sumBy(pendingRewards, reward =>
  ...
  public async getMaturityThresholds({ pid, provider }: { pid: string; provider: BaseProvider }) {
    ...
    const maturityThresholds = poolLevelInfo.requiredMaturities.map((maturity: BigNumber) =>
      maturity.toString()
    )
