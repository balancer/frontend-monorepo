...
export function useReliquaryJoinGetBptOutAndPriceImpactForTokensIn() {
  ...
  const tokenAmountsIn = tokenAmountsGetArrayFromMap(inputAmounts).map(({ amount, address }) => {
    ...
    const investOption = pool.investConfig.options.find(
      option => option.poolTokenIndex === poolTokenIndex
    )

    return {
      amount,
      address:
        selectedOptions[`${poolTokenIndex}`] || investOption?.tokenOptions[0].address || address,
    }
  })
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      if (
        !tokenAmountsIn ||
        tokenAmountsIn.every(
          tokenAmount => !tokenAmount.amount || parseFloat(tokenAmount.amount) === 0
        )
      ) {
      ...
      return poolService.joinGetBptOutAndPriceImpactForTokensIn(
        replacedWethTokenAmountsIn,
        slippage
      )
    },
  ...
  const hasMediumPriceImpact =
    !hasHighPriceImpact && priceImpact > networkConfig.priceImpact.invest.noticeable
