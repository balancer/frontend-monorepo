...
export function useAllRelicsDepositBalances() {
  ...
  for (const relic of relics) {
    ...
    allRelicsWftmAmount = allRelicsWftmAmount.add(
      parseUnits(relic.tokenAmounts[0]?.amount || '0', 18)
    )
    allRelicsBeetsAmount = allRelicsBeetsAmount.add(
      parseUnits(relic.tokenAmounts[1]?.amount || '0', 18)
    )
  }
