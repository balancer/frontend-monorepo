...
export function ReliquaryWithdrawSingleAsset({ onShowPreview, ...rest }: Props) {
  ...
  const { singleAssetWithdraw, setSingleAssetWithdrawAmount, setSingleAssetWithdraw } =
    useReliquaryWithdrawState()
  const singleAssetWithdrawForBptIn = useReliquaryExitGetSingleAssetWithdrawForBptIn()
  const { hasHighPriceImpact, formattedPriceImpact } =
    useReliquaryExitGetBptInForSingleAssetWithdraw()
  const [acknowledgeHighPriceImpact, { toggle: toggleAcknowledgeHighPriceImpact }] =
    useBoolean(false)
  ...
  const selectedTokenOption =
    tokenOptions.find(tokenOption => tokenOption.address === singleAssetWithdraw.address) ||
    tokenOptions[0]
  ...
  return (
    <Box pt={4} {...rest}>
      <Box mb="4">
        <Text>
          Select your desired asset, then drag the slider or enter an amount to configure your
          withdraw.
        </Text>
      </Box>
